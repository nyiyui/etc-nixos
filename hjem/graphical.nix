...
in
{
  ...
  config = {
    ...
    xdg.config.files."waybar/config" = {
      ...
      in
      {
        ...
        "mpris" = {
          ...
          dynamic-len = 40;
          player-icons = { firefox = "ff"; mpv = "mpv"; };
          status-icons = { playing = "生"; paused = "停"; stopped = "止"; };
          rotate = rotationAngle;
        };
        "custom/light" = if cfg.hasBacklight then {
          exec = "${pkgs.light}/bin/light";
          interval = 10;
          rotate = rotationAngle;
        } else null;
        
        "custom/next-event" = if cfg.icsUrlPath != null then {
          exec = "${
    ...
    files.".config/autostart/firefox.desktop".source = "/run/current-system/sw/share/applications/firefox.desktop";
    files.".config/autostart/signal.desktop".source = "/run/current-system/sw/share/applications/signal.desktop";
    files.".config/autostart/thunderbird.desktop".source = "/run/current-system/sw/share/applications/thunderbird.desktop";
    files.".config/autostart/io.github.alainm23.planify.desktop".source = "/run/current-system/sw/share/applications/io.github.alainm23.planify.desktop";
    files.".config/autostart/debug-autostart-env.desktop".source = "${debugEnvDesktop}/share/applications/debug-autostart-env.desktop";
