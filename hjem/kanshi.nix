  ...
  config =
    let
      ...
      settings = [
        {
          ...
          profile.outputs = [
            {
              criteria = "Sceptre Tech Inc U27 Unknown";
              position = "0,0";
              transform = "270";
            ...
            {
              criteria = "Sony SONY TV  *00 0x01010101";
              position = "900,160";
              mode = "3840x2160@30.000Hz";
              scale = 1.5;
            }
      ...
      mkOutput =
        out:
        let
          props = lib.attrsets.mapAttrsToList (
            k: v: if k == "criteria" then "" else "${k} ${builtins.toString v}"
          ) out;
        in
