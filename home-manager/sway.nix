{ config, libs, pkgs, lib, ... }: {
  imports = [
    ...
    ({ config, lib, pkgs, ... }: {
      config.programs.waybar.settings.mainBar = let
        ...
        '';
      in lib.mkIf config.kiyurica.graphical.onScreenKeyboard.enable {
        modules-left = [ "custom/keyboard-toggle" "custom/fuzzel-launch" ];
  ...
  options.kiyurica.graphical.onScreenKeyboard.enable =
    lib.mkEnableOption "on-screen keyboard";
  ...
  config.wayland.windowManager.sway = let modifier = "Mod4";
  in {
    enable = true;
  ...
  config.home.packages = with pkgs; [ pavucontrol swaylock ];
  ...
  config.systemd.user.services.swayidle =
    lib.mkIf config.kiyurica.graphical.idle {
      Unit = {
  ...
  config.systemd.user.services.swaybg =
    lib.mkIf config.kiyurica.graphical.background {
      Unit = {
      ...
      Service = {
        ExecStart =
          "${pkgs.swaybg}/bin/swaybg -mfill -i ${../wallpapers/umekita.jpg}";
        Restart = "on-failure";
        RestartSec = 3;
      };
      Install.WantedBy = [ "graphical-session.target" ];
  ...
  config.systemd.user.services.wvkbd =
