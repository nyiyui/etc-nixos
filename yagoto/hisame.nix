# hisame.nix syncs files with Fujitsu Quaderno A4 (Gen. 2) to a local folder.
{
  config,
  lib,
  pkgs,
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.hisame-sync = {
      ...
      serviceConfig = {
        ...
        }/bin/python -u ${pkgs.writeText "dptmount-generate-config.py" ''
          import subprocess
          import json
          import time
          import serial
